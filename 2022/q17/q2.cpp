...
int main(int ac, const char *av[]) {
    ...
    unsigned skip = 0;
    grid.push_back(full);
    map<tuple<unsigned,unsigned,unsigned>,pair<unsigned,unsigned>> memo;
    while(ctr <= 2022*2) {
	unsigned xpos = 2;
	auto z = memo.emplace(make_tuple(maxlevel, sh, gas), make_pair(skip,ctr));
	if (!z.second) {
	    clog << maxlevel << ' ' << sh << ' ' << gas << ' ' << skip << ' ' << ctr << endl;
	    clog << skip - z.first->second.first << ' ' << ctr - z.first->second.second << endl;
	    z.first->second.first = skip;
	    z.first->second.second = ctr;
	}
